...
[![Platforms](https://img.shields.io/badge/platform-macOS%20%7C%20Linux%20%7C%20Windows-4c7bb0)](https://github.com/AsagaKosho/kobunshoko/actions/workflows/ci.yml)
[![Python](https://img.shields.io/badge/python-3.12%2B-4c7bb0)](pyproject.toml)
...
※ 画像はすべて合成テストデータ（`tests/fixtures/`）です。

**一覧** — 取り込み・検索・絞り込み:

![一覧画面](docs/images/index.png)

**文書セット詳細** — メタデータ・改ざん検知バッジ・タグ・メモ・ファイル一覧:

![詳細画面](docs/images/detail.png)

**閲覧** — 様式XMLをサーバー側XSLT変換で表示（原本ダウンロード・PDF保存つき）:

![閲覧画面](docs/images/view.png)

## セットアップ

対応OS: **macOS / Linux / Windows**（3 OSでテストスイートをCI実行）。
